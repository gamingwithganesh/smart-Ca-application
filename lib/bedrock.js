...
export async function processWhatsAppMessage(incomingMessage, fromNumber, caId = null) {
  ...
  let session = await UserSession.findOne({ caId: effectiveCaId, phoneNumber: fromNumber });
  if (!session) {
    session = new UserSession({ caId: effectiveCaId, phoneNumber: fromNumber, welcomed: true });
    await session.save();
  ...
  const clientDocs = await Document.find({ clientId: client._id, uploadedBy: effectiveCaId }, 'documentType year fileUrl').lean();
